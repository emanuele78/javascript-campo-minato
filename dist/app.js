...
var GAME_MODULE = {
  ...
  timeElapsed: 0,
  currentLevel: 'easy',
  initializeGame: function initializeGame(difficulty) {
  ...
  addFlag: function addFlag(cellElement) {
  ...
  removeFlag: function removeFlag(cellElement) {
    cellElement.empty();
    this.bombsToFind++;
    printBombsCount();
  },
  showNearbyBombCount: function showNearbyBombCount(cellElement, nearbyBomb) {
    cellElement.html('<span class="color_' + nearbyBomb + '">' + nearbyBomb + '</span>');
  },
  endGameWithForBomb: function endGameWithForBomb(cellElement) {
  ...
  endGameForTimeExpired: function endGameForTimeExpired() {
    $('.header_face').html('<i class="fas fa-frown"></i>');
    var outer = this;
    setTimeout(function () {
      alert('Tempo scaduto');
  ...
  endGameForWinner: function endGameForWinner() {
    var outer = this;
    setTimeout(function () {
...
function checkWin() {
  var elements = $('.game_cell_hidden:has(.fa-flag)');
  var userWin = true;
  elements.each(function () {
    var rowIndex = $(this).parent().data('row-index');
...
function generateNewMap() {
  ...
  for (var i = 0; i < GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].rows; i++) {
    var cells = [];

    for (var j = 0; j < GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].cells_per_row; j++) {
      cells.push(0);
    }
...
function printCells() {
  var template = handlebars_dist_cjs_handlebars__WEBPACK_IMPORTED_MODULE_0___default.a.compile($('#grid-template').html());
...
function evaluateCellContent(clickedCellElement) {
  var rowIndex = clickedCellElement.parent().data('row-index');
  var cellIndex = clickedCellElement.data('cell-index');
...
function discoverFromCell(rowIndex, cellIndex) {
  ...
  var e = getCellFromCoordinates(rowIndex, cellIndex + 1);
...
function processElement(element, rowIndex, cellIndex) {
  if (element !== null && element.hasClass('game_cell_hidden') && element.is(':empty')) {
    var _nearbyBombsCount = GAME_MODULE.gameMap[rowIndex][cellIndex];

    if (_nearbyBombsCount === 0) {
      element.removeClass('game_cell_hidden'); //recursion

      discoverFromCell(rowIndex, cellIndex);
    } else if (_nearbyBombsCount > 0) {
      element.removeClass('game_cell_hidden');
      GAME_MODULE.showNearbyBombCount(element, _nearbyBombsCount);
    }
  }
}
/**
...
function getCellFromCoordinates(rowIndex, cellIndex) {
  ...
  var row = $('.row_wrapper[data-row-index=' + rowIndex + ']');
...
function placeBombs() {
  ...
  while (bombPlaceds <= GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].bombsCount) {
    var randomRow = Math.floor(Math.random() * GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].rows);
    var randomCell = Math.floor(Math.random() * GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].cells_per_row);
...
function calculateNearbyBombs() {
  for (var row = 0; row < GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].rows; row++) {
    for (var cell = 0; cell < GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].cells_per_row; cell++) {
...
function checkCoordinates(rowIndex, cellIndex) {
  return !(rowIndex < 0 || cellIndex < 0 || cellIndex >= rowIndex > GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].cells_per_row || rowIndex >= GAME_MODULE.gameSettings[GAME_MODULE.currentLevel].rows);
}
...
__webpack_require__(/*! /Users/emanuelemazzante/WorkingDirectory/Esercizi_Boolean/demo_minesweeper/src/app.js */"./src/app.js");
module.exports = __webpack_require__(/*! /Users/emanuelemazzante/WorkingDirectory/Esercizi_Boolean/demo_minesweeper/src/app.scss */"./src/app.scss");
